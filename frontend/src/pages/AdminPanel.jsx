...
export default function AdminPanel() {
  ...
  // Modals
  const [positionModal, setPositionModal] = useState(null); // null | { mode:'add'|'edit', data? }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="form-group">
            ...
            <input
              ...
              type="file"
              accept="image/*"
              onChange={e => { setPhoto(e.target.files[0]); setRemovePhoto(false); }}
              style={{ border: 'none', padding: 0, background: 'transparent' }}
            />

            {/* Newly selected file — show filename + remove button */}
            {photo && (
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: '0.8rem', color: 'var(--gray-600)' }}>📷 {photo.name}</span>
                <button type="button" className="btn-danger" style={{ fontSize: '0.78rem', padding: '4px 10px' }}
            ...
            {/* Existing saved photo — preview + change/remove options */}
            {editing && candidateModal.data.photo_url && !photo && !removePhoto && (
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                <img src={candidateModal.data.photo_url} alt="current"
                  style={{ width: 60, height: 60, borderRadius: '50%', objectFit: 'cover' }} />
                <button type="button" className="btn-danger" style={{ fontSize: '0.78rem', padding: '5px 12px' }}
            ...
            {/* Removal confirmed — show undo */}
            {removePhoto && !photo && (
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: '#fef2f2', borderRadius: 'var(--radius-sm)', border: '1px solid #fecaca' }}>
                <span style={{ fontSize: '0.82rem', color: 'var(--red)' }}>Photo will be removed on save.</span>
                <button type="button" className="btn-ghost" style={{ fontSize: '0.78rem', padding: '4px 10px' }}
  ...
  const VoterModal = () => {
    const [form, setForm] = useState({ matric_number: '', name: '' });
    const [saving, setSaving] = useState(false);
  ...
  const BulkVoterModal = () => {
    ...
    const parseFile = (f) => {
      ...
      reader.onload = (e) => {
        ...
        const hasHeader = ['matric', 'matric_number', 'matric number', 'no'].includes(firstCell);
    ...
    const save = async () => {
      if (!preview.length) return flash('error', 'No valid rows to import');
      setSaving(true);
      try {
    ...
    return (
      <Modal
        ...
        footer={<>
          <button className="btn-ghost" onClick={() => setBulkModal(false)}>Cancel</button>
          <button className="btn-primary" onClick={save} disabled={saving || !preview.length}>{saving ? 'Importing...' : `Import ${preview.length} Voters`}</button>
        </>}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <p style={{ fontSize: '0.85rem', color: 'var(--gray-600)' }}>
            Upload a <strong>CSV or Excel (.csv / .tsv)</strong> file with columns:<br />
            <code style={{ fontSize: '0.8rem' }}>Matric Number, Full Name, Level</code><br />
          ...
          />
          {preview.length > 0 && (
            <div style={{ maxHeight: 200, overflowY: 'auto', border: '1px solid var(--gray-200)', borderRadius: 'var(--radius-sm)' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                <thead>
                  <tr style={{ background: 'var(--gray-100)', position: 'sticky', top: 0 }}>
                    <th style={{ padding: '6px 10px', textAlign: 'left' }}>Matric</th>
                    <th style={{ padding: '6px 10px', textAlign: 'left' }}>Name</th>
                    <th style={{ padding: '6px 10px', textAlign: 'left' }}>Level</th>
                  </tr>
                </thead>
                <tbody>
                  {preview.map((v, i) => (
                    <tr key={i} style={{ borderTop: '1px solid var(--gray-100)' }}>
                      <td style={{ padding: '5px 10px', fontFamily: 'monospace' }}>{v.matric_number}</td>
                      <td style={{ padding: '5px 10px' }}>{v.name}</td>
                      <td style={{ padding: '5px 10px', color: 'var(--gray-600)' }}>{v.level || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {file && !preview.length && (
            <p style={{ fontSize: '0.82rem', color: 'var(--red)' }}>No valid rows found. Check that columns are in the right order.</p>
          )}
        </div>
      </Modal>
    );
  ...
  const resetVote = async (id) => {
    if (!window.confirm('Reset this voter\'s vote? Use only in emergencies.')) return;
