...
// Voter login — restricted to pre-imported list only
router.post('/voter/login', checkSiteEnabled, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      'SELECT * FROM voters WHERE UPPER(matric_number) = $1',
      [matric]
    );

    if (!rows.length) {
      return res.status(403).json({
        error: 'You are not on the voter list. Contact the electoral committee.',
        not_registered: true,
      });
    }
...
// Lookup voter by matric (for the confirm-identity step on login page, no token needed)
router.get('/voter/lookup', checkSiteEnabled, async (req, res) => {
  const matric = (req.query.matric || '').trim().toUpperCase();
  if (!matric) return res.status(400).json({ error: 'Matric number required' });

  try {
    const { rows } = await pool.query(
      'SELECT matric_number, name, level, has_voted FROM voters WHERE UPPER(matric_number) = $1',
      [matric]
    );
    if (!rows.length) {
